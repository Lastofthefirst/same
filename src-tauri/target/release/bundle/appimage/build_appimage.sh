...
mkdir -p "same-mouth.AppDir"
cp -r ../appimage_deb/data/usr "same-mouth.AppDir"

cd "same-mouth.AppDir"
mkdir -p "usr/bin"
mkdir -p "usr/lib"
...
( cd "/home/runner/.cache/tauri" && ( wget -q -4 -N https://github.com/AppImage/AppImageKit/releases/download/continuous/AppRun-${ARCH} || wget -q -4 -N https://github.com/AppImage/AppImageKit/releases/download/12/AppRun-${ARCH} ) )
chmod +x "/home/runner/.cache/tauri/AppRun-${ARCH}"

# We need AppRun to be installed as same-mouth.AppDir/AppRun.
# Otherwise the linuxdeploy scripts will default to symlinking our main bin instead and will crash on trying to launch.
cp "/home/runner/.cache/tauri/AppRun-${ARCH}" AppRun

cp "usr/share/icons/hicolor/1x1@2/apps/same-mouth.png" .DirIcon
ln -sf "usr/share/icons/hicolor/1x1@2/apps/same-mouth.png" "same-mouth.png"

ln -sf "usr/share/applications/same-mouth.desktop" "same-mouth.desktop"
...
( cd "/home/runner/.cache/tauri" && wget -q -4 -N https://raw.githubusercontent.com/tauri-apps/linuxdeploy-plugin-gtk/master/linuxdeploy-plugin-gtk.sh )
( cd "/home/runner/.cache/tauri" && wget -q -4 -N https://github.com/tauri-apps/binary-releases/releases/download/linuxdeploy/linuxdeploy-${linuxdeploy_arch}.AppImage )

chmod +x "/home/runner/.cache/tauri/linuxdeploy-plugin-gtk.sh"
chmod +x "/home/runner/.cache/tauri/linuxdeploy-${linuxdeploy_arch}.AppImage"

dd if=/dev/zero bs=1 count=3 seek=8 conv=notrunc of="/home/runner/.cache/tauri/linuxdeploy-${linuxdeploy_arch}.AppImage"

OUTPUT="same-mouth_0.1.0_amd64.AppImage" "/home/runner/.cache/tauri/linuxdeploy-${linuxdeploy_arch}.AppImage" --appimage-extract-and-run --appdir "same-mouth.AppDir" --plugin gtk ${gst_plugin} --output appimage
